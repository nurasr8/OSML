...
class LauncherWindow(wx.Frame):
    def __init__(self) -> None:
        ...
        self.version_row_label = wx.StaticText(main_panel, label="Version")
        version_sizer.Add(self.version_row_label, 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 10)
        ...
        version_sizer.Add(self.version_combo, 1, wx.EXPAND)
        main_sizer.Add(version_sizer, 0, wx.ALL | wx.EXPAND, 10)

        # Username selection
        username_sizer = wx.BoxSizer(wx.HORIZONTAL)
        self.username_row_label = wx.StaticText(main_panel, label="Username")
        username_sizer.Add(self.username_row_label, 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 10)
        ...
        # Debug panel
        self.debug_frame = wx.StaticBoxSizer(wx.VERTICAL, main_panel, "Debug Info")
    ...
    def apply_language(self) -> None:
        ...
        self.start_button.SetLabel(self.tr("start_game"))
        if self.choices:
    ...
    def on_settings(self, event: wx.Event) -> None:
        dialog = wx.Dialog(self, title=self.tr("settings_title"), size=(400, 300))
        ...
        current_index = (
            self._language_codes.index(self.settings.language)
            if self.settings.language in self._language_codes
            else 0
        )
        ...
        if dialog.ShowModal() == wx.ID_OK:
            selected_index = self.language_combo.GetSelection()
            selected_language = (
            ...
            self.settings = LauncherSettings(
                ram_mb=int(self.ram_spin.GetValue()) * 1024,
                window_width=int(self.window_width_spin.GetValue()),
                window_height=int(self.window_height_spin.GetValue()),
                language=selected_language,
                debug_enabled=self.debug_check.GetValue(),
            )
            try:
                self.settings.save()
                self.apply_language()
